  ...
  return (
    <>
      <header className="flex justify-center p-5 py-10 fixed w-full z-[99999999]">
        <div className="w-[1000px] inline-flex justify-between items-center border dark:bg-slate-800/20 dark:border-white/10 dark:text-white backdrop-blur-xl bg-white p-5 py-3 rounded-2xl">
          <Logo width="50px" />
          <nav
            className={`${
              isMenuOpen ? "block" : "hidden"
            } xl:block fixed w-full left-0 top-32 xl:mt-0 xl:static xl:w-fit xl:ml-20`}
          >
            <ul className="flex gap-5 text-sm relative font-medium dark:bg-slate-900 dark:border-white/20 xl:!bg-transparent bg-white border dark:text-white/70 text-color-1/70 p-5 rounded-xl flex-col xl:flex-row xl:p-0 xl:border-0">
              {NavData &&
          ...
          <div className="flex items-center gap-5 xl:w-[150px] justify-end">
            <div
              className={`border dark:border-white/20 p-1 h-fit scale-75 rounded-xl xl:hidden ${
                isMenuOpen ? "dark:bg-white/20 bg-black text-white" : ""
              }`}
            >
              <Hamburger
                size={30}
